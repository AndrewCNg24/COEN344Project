...
parser.add_argument('--gaus', default=False, type=bool,
                    help='toggle gaussian filter')
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0 and args.rank == 0:
            ...
            debug_info(x_norm, args.l_a, args.u_a,
                           args.l_margin, args.u_margin, args.epoch)


def debug_info(x_norm, l_a, u_a, l_margin, u_margin, epoch):
    """
    ...
    print(' [{:.2f}] [debug info]: x_norm mean: {:.2f} min: {:.2f} max: {:.2f}'
          .format(epoch, mean_, min_, max_))
    print(' [{:.2f}] [debug info]: margin mean: {:.2f} min: {:.2f} max: {:.2f}'
          .format(epoch, m_mean_, m_min_, m_max_))
